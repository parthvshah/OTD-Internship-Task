...
def find_seq_common_route_pairs(unique_pairs, source_id, dest_id):
    ...
    for pair in unique_pairs:
        ...
        if(stop_seq1 < stop_seq2 and stop_seq3 <= stop_seq4):
            seq_pairs.append([source_route_id, dest_route_id,
                              (stop_seq1, stop_seq2, stop_seq3, stop_seq4)])
...
def query():
    if request.method == 'POST':
        ...
        if(zero_result == False):
            zero_result = [
                'There are no 0 hop routes for given source and destination.']
        else:
            zero_result = createZeroResult(zero_result)
        if(one_result == False):
            one_result = [
                'There are no 1 hop routes for given source and destination.']
        else:
